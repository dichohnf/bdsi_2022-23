...
CREATE PROCEDURE assegna_codice(IN tipo_codice ENUM('T','I','S','G','P','C'), INOUT codice VARCHAR(10))
	NOT DETERMINISTIC
	CASE tipo_codice
    WHEN 'T' THEN SET codice = concat('T-',codice);
    WHEN 'I' THEN SET codice = concat('I-',codice);
    WHEN 'S' THEN SET codice = concat('S-',codice);
    WHEN 'G' THEN SET codice = concat('G-',codice);
    WHEN 'P' THEN SET codice = concat('P-',codice);
    WHEN 'C' THEN SET codice = concat('C-',codice);
    WHEN 'F' THEN SET codice = concat('F-',codice);
    END CASE$$
